...
import { Reeve, Player } from '../types';
import { UserPlus, ShieldCheck, Trash2, Zap, Search } from 'lucide-react';

interface ReevesRosterProps {
  reeves: Reeve[];
...
const ReevesRoster: React.FC<ReevesRosterProps> = ({ 
  reeves, masterPlayers = [], onImportPlayer, onAdd, onRemove, onAddBulk, disabled 
}) => {
  ...
  const filteredSuggestions = useMemo(() => {
    ...
    return masterPlayers.filter(p => 
      p.roles.includes('reeve') && 
      p.name.toLowerCase().includes(query) &&
      !reeves.some(r => r.name.toLowerCase() === p.name.toLowerCase())
    ).slice(0, 5);
  }, [name, masterPlayers, reeves]);
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-app-primary-muted rounded-lg"><ShieldCheck className="w-5 h-5 text-app-primary" /></div>
          <h3 className="text-lg font-medieval text-app-text uppercase tracking-wider">War Reeves</h3>
        </div>
        ...
        <div className="relative z-40">
          <form onSubmit={handleSubmit} className="flex gap-2">
            <input 
              ...
              className="flex-1 bg-app-surface border-2 border-app-border rounded-xl px-4 py-2.5 text-sm text-app-text placeholder:text-app-text-muted focus:outline-none focus:border-app-primary/50 transition-all shadow-inner" 
            />
            <button type="submit" className="bg-app-primary hover:bg-sky-500 text-white p-2.5 rounded-xl transition-all shadow-lg active:scale-95"><UserPlus className="w-5 h-5" /></button>
          </form>
              ...
              {filteredSuggestions.map(player => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-bold text-app-text group-hover:text-app-primary transition-colors">{player.name}</span>
                  </div>
      ...
      <div className="space-y-1.5 max-h-[400px] overflow-y-auto pr-1.5 custom-scrollbar">
        {reeves.length === 0 ? (
          ...
          reeves.map(r => (
            <div key={r.id} className="flex items-center justify-between bg-app-surface border border-app-border rounded-xl px-3 py-2.5 group transition-all hover:border-app-primary/30">
              <div className="flex items-center gap-3">
                 <div className="w-6 h-6 bg-app-primary-muted rounded-lg flex items-center justify-center text-[10px] font-black text-app-primary">{r.id}</div>
                 <span className="text-xs font-bold text-app-text">{r.name}</span>
              </div>
              {!disabled && (
                <button onClick={() => onRemove(r.id)} className="text-app-text-muted/30 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-all">
